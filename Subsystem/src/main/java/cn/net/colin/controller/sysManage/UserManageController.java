...
public class UserManageController {
    ...
    public ResultInfo saveUser(SysUser user,String [] roleIds){
        ...
        try {
            ...
            lock = redisLock.lock(Constants.USERANDROLE_LOCK, 3, 30000);
        }catch (Exception e){
            e.printStackTrace();
            return resultInfo;
        }
    ...
    public ResultInfo updateHeadImg(String headImg,HttpServletRequest request){
        SysUser user = new SysUser();
        user.setHeadImg(RequestUtil.getBaseUrl(request)+headImg);
        SysUser sysUser = SpringSecurityUtil.getPrincipal();
        if (sysUser != null){
